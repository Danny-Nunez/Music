...
function processRenderer(renderer: YouTubeRenderer): YouTubeMusicItem | null {
  ...
  // Get playlistId from menu items first, then fall back to other locations
  let playlistId = '';
  
  // Try to get from menu items first
  if (renderer.menu?.menuRenderer?.items) {
    for (const item of renderer.menu.menuRenderer.items) {
      const watchPlaylistId = item.menuNavigationItemRenderer?.navigationEndpoint?.watchPlaylistEndpoint?.playlistId;
      if (watchPlaylistId) {
  ...
  // Fall back to other locations if menu items didn't have it
  if (!playlistId) {
