          ...
          <div className={`grid grid-cols-1 md:grid-cols-2 gap-6 transition-all duration-300 ease-in-out ${isSliding ? 'transform -translate-x-4 opacity-80' : 'transform translate-x-0 opacity-100'}`}>
            {(() => {
              ...
              const rightColumnSongs = currentSongs.slice(4, 8);
                 ...
                 <div key={`${categoryIndex}-column-${columnIndex}`} className="space-y-2">
                  {columnSongs.map((song, index) => {
                    const globalIndex = startIndex + (columnIndex * 4) + index;
                    return (
                                             <div
                         key={`${categoryIndex}-song-${song.videoId}-${globalIndex}`}
                         className="song-row flex items-center gap-3 p-3 rounded-lg bg-gray-950 border border-gray-900 hover:bg-gray-800/50 transition-colors cursor-pointer group"
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2 flex-shrink-0 add-to-playlist-container">
                                                     <div className="relative opacity-100 transition-all duration-200" key={`${categoryIndex}-playlist-btn-${song.videoId}-${globalIndex}`}>
                            <AddToPlaylistButton
                              track={{
