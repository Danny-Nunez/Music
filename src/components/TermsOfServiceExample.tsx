...
export default function TermsOfServiceExample() {
  // Method 1: Direct import (fastest, for static usage)
  const staticData = termsOfServiceData;
  
  // Method 2: API hook (for dynamic usage)
  const { data: apiData, loading, error } = useTermsOfService();

  return (
    <div className="p-4">
      <h2 className="text-xl font-bold mb-4">Terms of Service Data Access Methods</h2>
      ...
      <div className="mb-6">
        ...
        <div className="bg-gray-100 p-3 rounded">
          {staticData.sections.find(s => s.id === 'contact-information') && (
            <div>
              <h4 className="font-semibold">
                {staticData.sections.find(s => s.id === 'contact-information')?.title}
              </h4>
              <p>{staticData.sections.find(s => s.id === 'contact-information')?.content}</p>
              <ul className="list-disc pl-4 mt-2">
                {staticData.sections.find(s => s.id === 'contact-information')?.items?.map((item, idx) => (
                  <li key={idx}>{item}</li>
                ))}
              </ul>
            </div>
